class circleChart
  ...
  setup: () =>
    ...
    @nodes.each((d, i) =>
      ...
      unless i is 0
          followingF = @links.filter((d) -> d.source is i and (d.type is "ftof" or d.type is "mtof"))[0].length
          followingM = @links.filter((d) -> d.source is i and (d.type is "ftom" or d.type is "mtom"))[0].length
          @alltofData.push(followingF)
          @alltomData.push(followingM)
    ...
    meanALLTOM = d3.mean(@alltomData, (d) -> d)
    meanALLTOF = d3.mean(@alltofData, (d) -> d)
    ...
    console.log "MEAN all to f", meanALLTOF
    console.log "MEAN all to m", meanALLTOM

    console.log "INCOMING"
    console.log "MEAN f from m", incomingMeanMTF
    console.log "m from m", incomingMeanMTM
    console.log "MEDIAN f from m", incomingMedianMTF
    console.log "m from m", incomingMedianMTM
    console.log "MEAN f from f", incomingMeanFTF
    console.log "m from f", incomingMeanFTM
    console.log "MEDIAN f from f", incomingMedianFTF
    console.log "m from f", incomingMedianFTM
  ...
  start: =>
    ...
    @me.select('circle.inner').transition()
      .attr(r: @radius)
      .each('end', () =>
        @meOverlayText.transition().delay(400).attr(opacity: 1)
        ...
        @followOverlayText.transition().delay(3800).attr(opacity: 1)
...
d3.json("/data/bjeanes_data_equalised1.json", (e, data) ->
